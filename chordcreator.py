...
def create_midi_chord_progression(key, filename):
    ...
    midi.addKeySignature(track, time, key_signature['sharps_flats'], key_signature['mode'], key_signature['mode'])
...
# Main function to get user input and generate the MIDI file
def main():
    key_signature = input("Enter the key signature (e.g., C, G, D, Am, Em, etc.): ").strip()
    filename = input("Enter the filename to save the MIDI file (e.g., output.mid): ").strip()
